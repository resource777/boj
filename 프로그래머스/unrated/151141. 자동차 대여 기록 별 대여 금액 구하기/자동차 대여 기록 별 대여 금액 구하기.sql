...
CASE 
    ...
    THEN TRUNCATE((DATEDIFF(END_DATE,START_DATE)+1)*(
        100 - (SELECT DISCOUNT_RATE FROM CAR_RENTAL_COMPANY_DISCOUNT_PLAN D WHERE D.CAR_TYPE='트럭' AND D.DURATION_TYPE LIKE '%90%')
    )/100*DAILY_FEE,0)
    WHEN DATEDIFF(END_DATE,START_DATE)+1 BETWEEN 30 AND 89
    THEN TRUNCATE((DATEDIFF(END_DATE,START_DATE)+1)*(
        100 - (SELECT DISCOUNT_RATE FROM CAR_RENTAL_COMPANY_DISCOUNT_PLAN D WHERE D.CAR_TYPE='트럭' AND D.DURATION_TYPE LIKE '%30%')
    )/100*DAILY_FEE,0)
    WHEN DATEDIFF(END_DATE,START_DATE)+1 BETWEEN 7 AND 29
    THEN TRUNCATE((DATEDIFF(END_DATE,START_DATE)+1)*(
        100 - (SELECT DISCOUNT_RATE FROM CAR_RENTAL_COMPANY_DISCOUNT_PLAN D WHERE D.CAR_TYPE='트럭'AND D.DURATION_TYPE LIKE '%7%')
    )/100*DAILY_FEE,0) 
END
    AS FEE
